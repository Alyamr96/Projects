...
// Changes the tags of an existing expert after checking user making this request is an admin
...
module.exports.removeCompany = function(req, res, next) {  // Delete a company after checking the ID given.
    ...
    //Checking whether the company exists or not
...
//Viewing companies after checking that the user is an admin returns all companies
...
// Adding a company by checking input fields first then checking if the user is an admin then adding to DB
module.exports.addCompany = function(req, res, next) {      
//checking if the inputs are valid and of correct type
  var valid =
  ...
  ;
  //if not valid return a 422 code that inputs aren't all valid
  if (!valid) {
      return res.status(422).json({
          err: null,
...
//Check that the user exists and an admin
  User.findById(req.body.userid).exec(function(err,user) {
      if(err){
        return next(err);
      }
...
// Returns all companies that exist in the database
